...
export function defineTools(manager: SSHSessionManager) {
  return {
    "terminal_connect": async (args: unknown): Promise<CallToolResult> => {
    ...
    "terminal_execute": async (args: unknown): Promise<CallToolResult> => {
    ...
    "terminal_write": async (args: unknown): Promise<CallToolResult> => {
    ...
    "terminal_read": async (args: unknown): Promise<CallToolResult> => {
      ...
      const output = manager.readFromSession(
        parsed.sessionId,
        parsed.offset,
        parsed.limit
      );
    ...
    "terminal_disconnect": async (args: unknown): Promise<CallToolResult> => {
    ...
    "list_sessions": async (): Promise<CallToolResult> => {
    ...
    "terminal_status": async (): Promise<CallToolResult> => {
      const sessions = manager.listSessions();
      return {
        content: [
          {
...
export const toolDefinitions = [
  {
    ...
    description:
      "Open a new local terminal shell session on the ZimaOS server. Creates a bash shell running as the specified user. Use sessionId from the result for subsequent operations.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Execute a command in an existing terminal session. Waits for the command to complete and returns the output. Best for non-interactive commands.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "terminal_write",
    description:
      "Send raw input to an active terminal session. Use this for interactive input (e.g., typing passwords, sending Ctrl+C, arrow keys). For newlines, use '\\n'. For special keys, use ANSI sequences (e.g., '\\x03' for Ctrl+C).",
    inputSchema: {
      type: "object",
      properties: {
        sessionId: { type: "string", description: "Session ID from terminal_connect" },
  ...
  },
  {
    name: "terminal_read",
    description:
      "Read accumulated terminal output from a session. Supports pagination via offset and limit. Returns the current output buffer content.",
    inputSchema: {
      type: "object",
      properties: {
        sessionId: { type: "string", description: "Session ID from terminal_connect" },
